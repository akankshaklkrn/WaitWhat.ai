...
import ContextSection, { PresentationContext } from './ContextSection';
...
export default function LandingContent() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className={`bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 relative transition-all ${!uploadSuccess ? 'opacity-50' : ''}`}>
        {/* Step Number */}
        <div className={`absolute -top-4 -left-4 w-12 h-12 rounded-full flex items-center justify-center text-2xl font-bold shadow-lg ${uploadSuccess ? 'bg-white text-purple-600' : 'bg-gray-400 text-gray-600'}`}>
          2
        </div>
        ...
        <div className="space-y-3">
          ...
          <div 
            ...
            className={`p-4 rounded-xl border-2 transition-all flex items-center gap-4 ${!uploadSuccess ? 'cursor-not-allowed border-white/20 bg-white/5' : intensity === 0 ? 'border-white bg-white/20 shadow-lg cursor-pointer' : 'border-white/30 hover:border-white/50 hover:bg-white/10 cursor-pointer'}`}
          >
          ...
          <div 
            ...
            className={`p-4 rounded-xl border-2 transition-all flex items-center gap-4 ${!uploadSuccess ? 'cursor-not-allowed border-white/20 bg-white/5' : intensity === 1 ? 'border-white bg-white/20 shadow-lg cursor-pointer' : 'border-white/30 hover:border-white/50 hover:bg-white/10 cursor-pointer'}`}
          >
            <span className="text-3xl">🎯</span>
            <div className="flex-1">
          ...
          <div 
            ...
            className={`p-4 rounded-xl border-2 transition-all flex items-center gap-4 ${!uploadSuccess ? 'cursor-not-allowed border-white/20 bg-white/5' : intensity === 2 ? 'border-white bg-white/20 shadow-lg cursor-pointer' : 'border-white/30 hover:border-white/50 hover:bg-white/10 cursor-pointer'}`}
          >
            <span className="text-3xl">🔥</span>
            <div className="flex-1">
        ...
        <button 
          onClick={() => {
            if (uploadedFile && videoId) {
              try {
                if (context) {
                  sessionStorage.setItem(makeContextStorageKey(videoId), JSON.stringify(context));
                } else {
